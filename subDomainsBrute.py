...
class SubNameBrute:
    ...
    def _scan(self, j):
        self.resolvers[j].nameservers = [self.dns_servers[j % 2]]
...
if __name__ == '__main__':
    ...
    # make tmp dirs
    tmp_dir = 'tmp/%s_%s' % (args[0], int(time.time())) #tmp_dir: 'tmp/zju.edu.cn_int[start_time]'
    if not os.path.exists(tmp_dir):
        os.makedirs(tmp_dir)

    multiprocessing.freeze_support() # On Windows calling this function is necessary.
    all_process = []
    dns_servers = ['10.10.0.21','10.10.2.21']
